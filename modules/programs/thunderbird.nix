...
let
  ...
  user_prefs = fn: args: concatStrings (mapAttrsToList (name: value: ''
    user_pref("${name}", ${toJSON value});
  ''));

  mkUserJs = let
      user_pref = name: value: ''user_pref("${name}", ${toJSON value});'';
      user_prefs = fn: args: concatStringsSep "\n" (mapAttrsToList user_pref);
  ...
  in accs: foldAttrs (a:b: trace "duplicate key") (imap0 mkAccountPref accs);

  profileModule = with types; { config, ... }: {
    name = mkOption {
      type = nullOr str;
      default = null;
...
in {
  ...
  options = {
    ...
    programs.thunderbird = {
      ...
      profiles = mkOption {
        type = with types; attrsOf (submodule profileModule);
        default = { };
        description = ''
          profiles
        '';
